...
export default function SellerListings (): JSX.Element {
  const [addListingDialog, setaddListingDialog] = React.useState(false)
  const handleAddListing = (): void => {
    setaddListingDialog(true)
  }
  const closeDialog = (): void => { setaddListingDialog(false) }
  const { sellerId } = useParams()

  const [sellerItems, setSellerItems] = React.useState([])
  const { sendRequest } = useHttpClient()
  React.useEffect(() => {
    const getSellerItems = async (): Promise<void> => {
