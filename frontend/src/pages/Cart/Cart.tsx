...
import { CART_ITEMS, API_BASE_URL, COVERS } from "src/constants/endpoints";
      ...
      <>
        ...
        <Stack maxWidth="xl" className="grid">
          {cartItems.map((cartItem) => {
            ...
            return (
              <>
                <Paper sx={CART_ITEM_PAPER_STYLE} elevation={16}>
                  ...
                  <Box>
                    ...
                    <Typography variant="h5">
                      Price: ${cartItem.listing.price * cartItem.quantity} = $
                      {cartItem.listing.price} x {cartItem.quantity}
                    </Typography>
                    <Typography>
                    ...
                    <IconButton
                      onClick={deleteCartItem(cartItem.listing.listingId)}
                    >
                    ...
                    <IconButton
                      ...
                      disabled={
                        cartItem.quantity === cartItem.listing.inventory
                      }
                    >
        ...
        {cartItems.length > 0 && (
          <Paper
            sx={{ backgroundColor: "#1976d2", padding: 3, marginBottom: 20 }}
            elevation={24}
          >
            <Typography variant="h6" color="white">
              Subtotal: $
              {Math.round((cartTotalCost + Number.EPSILON) * 100) / 100}
            </Typography>
            <Typography variant="h6" color="white">
              Shipping: FREE
            </Typography>
            <Typography variant="h6" color="white">
              13% Tax (HST+GST): $
              {Math.round((0.13 * cartTotalCost + Number.EPSILON) * 100) / 100}
            </Typography>
            <Box
              sx={{
                height: 2,
            ...
            <Typography variant="h4" color="white">
              Total: $
              {Math.round((1.13 * cartTotalCost + Number.EPSILON) * 100) / 100}
            </Typography>
            <Button
              variant="contained"
              color="success"
