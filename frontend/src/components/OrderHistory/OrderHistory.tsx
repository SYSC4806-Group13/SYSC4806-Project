...
export default function OrderHistory() {
  const [orderHistoryItems, setOrderHistoryItems] = React.useState(
    Array<IOrderHistoryItem>
  );
          ...
          <Stack maxWidth="xl" className="grid">
            {orderHistoryItems.map((orderHistoryItem) => {
              const purchaseDateTime = new Date(
                orderHistoryItem.purchaseDateTime
              );
              return (
                <Paper sx={ORDER_HISTORY_ITEM_PAPER_STYLE} elevation={24}>
                  <Box sx={{ marginRight: 3 }}>
                    <img
                      src={
                        API_BASE_URL +
                        COVERS +
                        orderHistoryItem.listing.listingId
                      }
                      alt=""
                  ...
                  <Box>
                    ...
                    <Typography variant="h6">
                      Total Price: $
                      {orderHistoryItem.totalCartItemPriceAtPurchase}
                    </Typography>
                    <Typography variant="h6">
                      Purchased on: {purchaseDateTime.toLocaleDateString()} at{" "}
                      {purchaseDateTime.toLocaleTimeString()}
                    </Typography>
                  </Box>
                </Paper>
              );
